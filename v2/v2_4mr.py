...
df = pd.read_csv('stationary-syncs.csv')
mean_reverting_ids = []
bootstrap_ids = []
parameters_df = pd.DataFrame(columns=['pair_contract_address', 'mu', 'theta', 'sigma'])

i = 0
# Loop through each unique ID
for ts_id in df['pair_contract_address'].unique():
    sub_df = df[df['pair_contract_address'] == ts_id].reset_index(drop=True)
...
mean_reverting_df = df[df['pair_contract_address'].isin(mean_reverting_ids)]

mean_reverting_df.to_csv('mr-syncs.csv', index=False)

parameters_df.to_csv('mr-parameters.csv', index=False)